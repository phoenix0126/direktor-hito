<template>
  <tr>
    ...
    <td v-if="row['exercise'] === ':T'">
      <input v-model="material1"  type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded " placeholder="actividad" />
    </td>
    <td v-else>{{ row['exercise'] }}</td>
    <td v-if="row['restriction'] === ':T'">
      <input v-model="material"  type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded " placeholder="restriction" />
    </td>
    <td v-else>{{ row['restriction'] }}</td>
    <td v-if="row['restriction'] === ':T'">
    ...
    <td v-if="row['responsible'] === ':T'">
      <input v-model="material"  type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded " placeholder="restriction" />
    </td>
    <td v-else :class="{ 'hidden': hideCols.indexOf('responsible') > -1 }">{{ row['responsible'] }}</td>
    <td v-if="row['responsible'] === ':T'">
      <input v-model="material"  type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded " placeholder="restriction" />
    </td>
    <td v-else :class="{ 'hidden': hideCols.indexOf('responsible_area') > -1 }">{{ row['responsible_area'] }}</td>
    <td v-if="row['responsible'] === ':T'">
      <input v-model="material"  type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded " placeholder="restriction" />
      <img @click="handleClick('option')" src="../assets/images/icons/ic_arrow-down.svg"
          class="absolute top-1/2 -translate-y-1/2 right-2" alt=""
    ...
    <td v-else :class="{ 'hidden': hideCols.indexOf('condition') > -1 }">
      <span class="px-1 py-0.5 bg-[#EB9B00] rounded text-[10px] leading-3">En proceso</span>
    </td>
    <td v-if="row['responsible'] === ':T'">
      <input v-model="material"  type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded " placeholder="restriction" />
    </td>
    <td v-else :class="{ 'hidden': hideCols.indexOf('applicant') > -1 }">{{ row['applicant'] }}</td>
  </tr>
</template>
